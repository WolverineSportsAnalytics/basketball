...
if __name__ == "__main__":
    ...
    getPerformanceDataForEachPlayer = "SELECT playerID, dateID, fanduel, draftkings, fanduelPosition, draftkingsPosition, team, opponent FROM performance WHERE dateID = %s"
    ...
    get_ball_handlers = "Select blocks, points, steals, assists, turnovers, tRebounds, DDD, DD, 3PM, 3PA, oRebounds, dRebounds, minutes, FG, FGA, FT, FTA, BPM, PPM, SPM, APM, TPM, DDDPG, DDPG, 3PP, ORPM, DRPM, FGP, FTP, \"usage\", ORT, DRT, TS, eFG from team_vs_ball_handlers WHERE dateID = %s and dailyTeamID = %s"
    get_wings = "Select blocks, points, steals, assists, turnovers, tRebounds, DDD, DD, 3PM, 3PA, oRebounds, dRebounds, minutes, FG, FGA, FT, FTA, BPM, PPM, SPM, APM, TPM, DDDPG, DDPG, 3PP, ORPM, DRPM, FGP, FTP, \"usage\", ORT, DRT, TS, eFG from team_vs_wings where dateID = %s and dailyTeamID = %s"
    get_bigs = "Select blocks, points, steals, assists, turnovers, tRebounds, DDD, DD, 3PM, 3PA, oRebounds, dRebounds, minutes, FG, FGA, FT, FTA, BPM, PPM, SPM, APM, TPM, DDDPG, DDPG, 3PP, ORPM, DRPM, FGP, FTP, \"usage\", ORT, DRT, TS, eFG from team_vs_bigs WHERE dateID = %s and dailyTeamID = %s"
    get_centers = "Select blocks, points, steals, assists, turnovers, tRebounds, DDD, DD, 3PM, 3PA, oRebounds, dRebounds, minutes, FG, FGA, FT, FTA, BPM, PPM, SPM, APM, TPM, DDDPG, DDPG, 3PP, ORPM, DRPM, FGP, FTP, \"usage\", ORT, DRT, TS, eFG from team_vs_centers WHERE dateID = %s and dailyTeamID = %s"
    ...
    for player in playersPlaying:
            ...
            basicQueryData = (str(int(player[1]+1)), player[0])
            ...
            teamQuery = (str(int(player[1])+1), idTeam)
            cursor.execute(getTeamData, teamQuery)
            teamResult = cursor.fetchall()

            oppTeamQuery = (str(int(player[1])+1), idOppTeam)
            ...
            # doubl'n it!
            if player[4] == 'PG':
                pos_data = (str(int(player[1])+1), idOppTeam)
    ...
    theta = np.zeros(((np.shape(testX)[1])), 1)
