...
def projectById(startdateid, enddateid, cursor):
    getAllData = "select * from futures where fanduelPts is not null and dateID < " + enddateid + " and dateID >= " + startdateid + " ;"
    cursor.execute(getAllData)

    features = [list(feature) for feature in cursor.fetchall()] # matrix where each row is a feature for each player

    # How you would import and use ridge regression
    ridge = ridgeRegression(features)
    predictions = ridge.predict() # returns array of fanduel pt predictions mapping one to one with features
...
def insertproject(predictions, nameOfCol, cursor):
    for tuples in predictions:
        insertData = "UPDATE performance SET " + nameOfCol + " = " + tuples[2] + " WHERE playerID = " + tuples[0] + " AND dateID = " + tuples[1] + ";"
        cursor.execute(insertData)

def project(startDay, startMonth, startYear, endDay, endMonth, endYear, cursor):
