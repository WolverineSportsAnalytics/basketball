...
class FeatureSelector():
    ...
    def identify_single_unique(self):
        ...
        record_single_unique = pd.DataFrame(unique_counts[unique_counts == 1]).reset_index(
    ...
    def identify_collinear(self, correlation_threshold, one_hot=False):
        ...
        # Iterate through the columns to drop to record pairs of correlated
        # features
    ...
    def identify_all(self, selection_params):
        ...
        for param in [
            ...
            if param not in selection_params.keys():
                raise ValueError(
                    '%s is a required parameter for this method.' %
                    param)
    ...
    def plot_collinear(self, plot_all=False):
        ...
        if plot_all:
            ...
            # columns (x-axis) are features to drop and rows (y_axis) are
            # correlated pairs
        ...
        ax.set_yticklabels(
            list(
                corr_matrix_plot.index), size=int(
                160 / corr_matrix_plot.shape[0]))
        ...
        ax.set_xticklabels(
            list(
                corr_matrix_plot.columns), size=int(
                160 / corr_matrix_plot.shape[1]))
    ...
    def plot_feature_importances(self, plot_n=15, threshold=None):
        ...
        # Need to adjust number of features if greater than the features in the
        # data
        ...
        if threshold:
            ...
            # np.where returns the index so need to add 1 to have correct
            # number
            importance_index = np.min(
                np.where(
                    self.feature_importances['cumulative_importance'] > threshold))
            plt.vlines(
                x=importance_index + 1,
                ymin=0,
                ymax=1,
                linestyles='--',
                colors='blue')
